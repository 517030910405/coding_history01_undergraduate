...
struct node{
	int l,r,sz;
	LL val,a,b,sum;//y=ax+b
	bool sign;
	int randnum;
	node():l(0),r(0),sz(1),val(0),a(0),b(0),sum(0),sign(false),randnum(0){}
};
...
inline void pushdown(int p){
	...
	if (v[p].l!=0){
		...
		v[p1].b=v[p].b;
		v[p1].a=v[p].a;
		v[p1].sum=v[p1].b*v[p1].sz+v[p1].a*v[p1].sz*(v[p1].sz+1)/2;
		cnt+=v[p1].sz;
	}
...
void change(int p,int l,int r,LL a,LL b){
	if (l<=1&&v[p].sz<=r){
		...
		v[p].b=(1-l)*a+b;
		v[p].sign=true;
		v[p].sum=v[p].b*v[p].sz+v[p].a*v[p].sz*(v[p].sz+1)/2;
		return;
	}
