...
extern int ai_side;
std::string ai_name="Your_ai_name_here_p,k_random";
double want[3][2]={{0,0},{0.7,0},{1.1,0.05}};
int ran;
int locx, locy; 
int d = 0;
struct place{
		...
		for (int f2=2;f2<=17;f2++){
			if ((myw>0)&&(f1+f2)%2==1&&checkwall(f1,f2)){
				...
				if (minus2>=minus&&minus2<(1<<29)&&(len[myl]<(1<<29))){
					...
					if (-possible.score-want[ran][0]+want[ran][1]*myw>ans.score){
						ans.score=-possible.score-want[ran][0]+want[ran][1]*myw;
