...
if True:
    ...
    tf.add_to_collection(tf.GraphKeys.WEIGHTS, B4)
...
if True:
    ...
    train_step1 = tf.train.AdamOptimizer(.5e-2).minimize(cross_entropy+reg_term)
    train_step2 = tf.train.AdamOptimizer(.1e-2).minimize(cross_entropy+reg_term)
    train_step3 = tf.train.AdamOptimizer(.2e-3).minimize(cross_entropy+reg_term)
...
if True:
    for i in range(1500+1):
        ...
        if i % 50 == 0:
            ts = Batch.test2_batch(0,200)
            train_accuracy = accuracy.eval(feed_dict={X: ts[1], Y: ts[0], DropProb: 1.0},session = sess)
            print('step %d, eval accuracy %.5g' % (i, train_accuracy))
            crss_entrp = cross_entropy.eval(feed_dict={X: ts[1], Y: ts[0], DropProb: 1.0},session = sess)
            print('eval_cross_entropy = %.5g' % (crss_entrp))
            ts = Batch.train2_batch(200)
            train_accuracy = accuracy.eval(feed_dict={X: ts[1], Y: ts[0], DropProb: 1.0},session = sess)
            print('trainning accuracy %.5g' % ( train_accuracy))
            crss_entrp = cross_entropy.eval(feed_dict={X: ts[1], Y: ts[0], DropProb: 1.0},session = sess)
            print('training_cross_entropy = %.5g\n' % (crss_entrp))
            #print("")
        #train_step.run( feed_dict={X: bt[1], Y: bt[0], DropProb: 0.5},session = sess)
        if (i<500):
            sess.run([train_step1],feed_dict={X: bt[1], Y: bt[0], DropProb: 0.5})
        else:
            if (i<1000):
                sess.run([train_step2],feed_dict={X: bt[1], Y: bt[0], DropProb: 0.5})
            else:
                sess.run([train_step3],feed_dict={X: bt[1], Y: bt[0], DropProb: 0.5})
